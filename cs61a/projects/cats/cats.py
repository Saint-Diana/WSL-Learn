...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def iscontained(paragraph):
        """This function takes a paragraph and returns a boolean 
        indicating whether that paragraph contains any of the words in topic.
        
        Note: Ignore the uppercase and lowercase.
        """
        # 首先去除paragraph中不是字母和空格的元素
        cleaned_chars = [char for char in paragraph if char.isalpha() or char == ' ']
        paragraph = ''.join(cleaned_chars)
        # 然后将其转换为单词列表
        words = list(map(lower, paragraph.split()))
        for t in topic:
            if t.lower() in words:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    lt, lr, count = len(typed_words), len(reference_words), 0
    if lt == 0 or lr == 0:
        return 0.0
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    # 先求出WPS，再乘上60就可以求出WPM
    return round(len(typed) / elapsed / 5 * 60, 2)
    # END PROBLEM 4
